...
namespace SolidEdgeSDK.AddIn
{
    ...
    public abstract class SolidEdgeAddIn : MarshalByRefObject, SolidEdgeFramework.ISolidEdgeAddIn
    {
        ...
        public EdgeBarPage AddWpfEdgeBarPage<TControl>(EdgeBarPageConfiguration config, SolidEdgeFramework.SolidEdgeDocument document = null) where TControl : System.Windows.Controls.Page, new()
        {
            ...
            var edgeBarPage = AddEdgeBarPage<EdgeBarPage>(
                config: config,
...
{
    #region  Ù–‘¿©’π
    public class SolidEdgeDocumentEx
    {
        public SolidEdgeDocumentEx() { }
        public void GetSetProperties(SolidEdgeFramework.Application application)
        {
            string meg = "";
            SolidEdgeFramework.SolidEdgeDocument curDoc = application.ActiveDocument;

            if (curDoc == null)
                return;

            SolidEdgeFramework.PropertySets propertieSets = curDoc.Properties;
            if (propertieSets == null)
                return;

            int nPropertiesCount = propertieSets.Count;
            for (long j = 1; j <= nPropertiesCount; ++j)
            {

                SolidEdgeFramework.Properties properties = propertieSets.Item(j);
                if (properties == null)
                    continue;

                string proSetsName = properties.Name;
                if (proSetsName != "Custom")
                    continue;

                long nCustomBomCount = properties.Count;
                for (long i = 1; i <= nCustomBomCount; ++i)
                {
                    SolidEdgeFramework.PropertyEx propPtr = (SolidEdgeFramework.PropertyEx)properties.Item(i);
                    if (propPtr == null)
                        continue;
                    try
                    {
                        ...
                        propPtr.GetProps(out name, out getvalue, out type);
        ...
        public List<MyProperties> GetProperties(SolidEdgeFramework.SolidEdgeDocument curDoc)
        {
            List<MyProperties> myProperties = new List<MyProperties>();
            string meg = "";
            // SolidEdgeFramework.SolidEdgeDocument curDoc = application.ActiveDocument;
            ...
            for (long j = 1; j <= nPropertiesCount; ++j)
            {
                ...
                for (long i = 1; i <= nCustomBomCount; ++i)
                {
                    ...
                    try
                    {
                        ...
                        myProperties.Add(new MyProperties { name = name, value = getvalue });
